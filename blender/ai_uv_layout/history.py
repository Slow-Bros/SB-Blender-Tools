...
laesst sich nicht abbrechen (siehe docs/ai-retopo.md, Cancelling). Stirbt
Blender waehrend er laeuft, ist das Ergebnis nur dann noch erreichbar, wenn die
Job-Id die Sitzung ueberlebt hat. Deshalb steht der Eintrag hier, sobald die
Id da ist, und nicht erst nach dem Download.
...
werden, traegt jeder solche Eintrag die Kennung seines Dokuments (SESSION).

Bis auf die Namen der Properties ist dieses Modul das des Retopo-Add-ons.
"""
...
def directory():
    ...
    try:
        ...
        return bpy.utils.user_resource("CONFIG", path="sb_ai_uv_layout", create=True)
...
def sync(context=None):
    ...
    if wm is None or not hasattr(wm, "sb_ai_uv_history"):
        return 0

    settings = getattr(context.scene, "sb_ai_uv", None) if context.scene else None
    ...
    wm.sb_ai_uv_history.clear()
    for entry in data:
        item = wm.sb_ai_uv_history.add()
    ...
    wm.sb_ai_uv_history_index = min(
        wm.sb_ai_uv_history_index, max(0, len(wm.sb_ai_uv_history) - 1)
    )
    return len(data)
...
def selected(context):
    ...
    items = getattr(wm, "sb_ai_uv_history", None)
    if not items:
        return None
    index = wm.sb_ai_uv_history_index
