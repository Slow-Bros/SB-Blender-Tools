...
"""Jobs: Export -> Scenario API (Worker-Thread) -> UV-Transfer.
...
Bis auf die Namen und den Transfer am Ende ist dieses Modul das des
Retopo-Add-ons.
"""
...
def _worker(job, api_key, api_secret, obj_path, model_spec, poll_interval):
...
def _import(context, job):
    ...
    try:
        ...
        log(traceback.format_exc())
        _end(job, error=f"UV transfer failed: {e}", settings=settings)
        return
...
def _scene_settings():
    ...
    return getattr(scene, "sb_ai_uv", None) if scene else None
...
class SB_OT_ai_uv_layout(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        _start(
            job, _worker,
            (job, api_key, api_secret, obj_path, spec, float(prefs.poll_interval)),
            "sb_ai_uv_layout",
        )
        return {"FINISHED"}


class SB_OT_ai_uv_layout_history_import(bpy.types.Operator):
...
class SB_OT_ai_uv_layout_cancel(bpy.types.Operator):
    ...
    bl_label = "Cancel"
    bl_description = "Stop waiting for this UV job. The job itself keeps running at Scenario"
