...
Ablauf wie im UV-Schritt von Phototron (apps/desktop/public/ipc/retopology.js):
  ...
  4. POST /v1/generate/custom/{model_id} mit der Asset-Id
  5. GET /v1/jobs/{id} bis success
  6. GET /v1/assets/{id} -> url -> Download, bevorzugt OBJ

Alle Netzwerkaufrufe laufen in einem Worker-Thread; hier wird KEIN bpy verwendet.
Der Client ist bis auf die Meldungen derselbe wie im Retopo-Add-on.
"""
...
# Reihenfolge der Bevorzugung beim Herunterladen. Phototron fordert fuer das
# UV-Ergebnis ausdruecklich OBJ an: es behaelt Quads und die Reihenfolge der
# Faces, und auf beides stuetzt sich der UV-Transfer per Index.
...
class ScenarioClient:
    ...
    def upload_3d(self, data, file_name, content_type, on_progress=None,
        ...
        max_bytes ist das Limit des Modells, das die Datei bekommt.
        """
        if len(data) > max_bytes:
            raise ScenarioError(
                f"File too large ({len(data) / 1024 / 1024:.0f} MB, limit is "
                f"{max_bytes / 1024 / 1024:.0f} MB). The UV model needs the mesh "
                "unchanged, so reduce it before unwrapping."
            )
    ...
    def start_generation(self, model_id, body):
        ...
        Der Body kommt von models.build_request.
        """
        if not model_id:
    ...
    def download_job_mesh(self, job_result):
        """Laedt das 3D-Ergebnis eines Jobs. Bevorzugt OBJ, sonst GLB, sonst FBX.
